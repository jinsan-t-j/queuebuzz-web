...
async function initialize() {
    try {
        ...
        hasError.value = true
        emit('error', err?.message || 'Failed to start camera')
    }
}
...
<template>
  <div class="fixed inset-0 z-[100] flex flex-col bg-plum text-white overflow-hidden animate-in fade-in duration-300">
    <!-- Header -->
    ...
    <div class="relative flex-1 flex flex-col items-center justify-center px-6 pb-20">
        ...
        <div v-if="hasError" class="flex flex-col items-center gap-6 max-w-xs text-center">
            ...
            <div>
                ...
                <p class="font-body text-sm text-plum-muted leading-relaxed">
                    We couldn't access your camera. Please ensure permissions are granted in your browser settings.
                </p>
            </div>
        ...
        >
            <div :id="scannerElementId" class="w-full h-full object-cover"></div>
            
            <!-- Scanning Overlay Decorations -->
            <div class="absolute inset-0 border-[2px] border-mint/30 pointer-events-none rounded-[40px]" />
            
            <!-- Corner Frames -->
            <div class="absolute top-8 left-8 w-8 h-8 border-t-4 border-l-4 border-mint rounded-tl-lg" />
            <div class="absolute top-8 right-8 w-8 h-8 border-t-4 border-r-4 border-mint rounded-tr-lg" />
            <div class="absolute bottom-8 left-8 w-8 h-8 border-b-4 border-l-4 border-mint rounded-bl-lg" />
            <div class="absolute bottom-8 right-8 w-8 h-8 border-b-4 border-r-4 border-mint rounded-br-lg" />
            
            <!-- Scanning Line -->
            <div class="absolute top-0 inset-x-0 h-[2px] bg-mint/50 shadow-[0_0_15px_rgba(0,229,160,0.8)] animate-[scan_3s_infinite_linear]" />
        </div>

        <!-- Help Text -->
        <div v-if="isScanning" class="mt-12 flex items-center gap-3 bg-white/5 px-4 py-2.5 rounded-2xl animate-in slide-in-from-bottom-4 duration-500">
