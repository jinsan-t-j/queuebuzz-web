...
const { isLoading, error, fetchHistoryDetail } = useHistoryApi()
...
function downloadCsv() {
  ...
  const csvRows = [
    ...
    ...queueDetail.value.entries.map(c => 
      [c.ticket, c.name, c.status, c.waited, c.joined, c.servedAt]
        .map(v => `"${v}"`)
        .join(',')
    ),
  ]
...
<template>
  <div class="p-6 md:p-8 space-y-8 bg-sand min-h-screen">
    ...
    <div class="flex items-center gap-4">
      <BaseButton variant="ghost" size="sm" @click="goBack" class="rounded-xl">
    ...
    <div v-if="queueDetail" class="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
      <BaseCard class="p-5 border-l-4 border-l-plum">
        <p class="font-body text-[10px] uppercase font-bold tracking-widest text-plum-muted mb-1">Total Bookings</p>
      ...
      <BaseCard class="p-5 border-l-4 border-l-mint">
        <p class="font-body text-[10px] uppercase font-bold tracking-widest text-plum-muted mb-1">Served</p>
        <div class="flex items-end gap-2">
      ...
      <BaseCard class="p-5 border-l-4 border-l-danger">
        <p class="font-body text-[10px] uppercase font-bold tracking-widest text-plum-muted mb-1">Skipped / Drops</p>
        <div class="flex items-end gap-2">
          <span class="font-mono text-3xl font-bold text-plum">{{ queueDetail.stats.droppedNoShow }}</span>
          <XCircleIcon class="w-5 h-5 text-danger mb-1.5" />
        </div>
      </BaseCard>

      <BaseCard class="p-5 border-l-4 border-l-plum-soft">
        <p class="font-body text-[10px] uppercase font-bold tracking-widest text-plum-muted mb-1">Avg. Wait Time</p>
        <div class="flex items-end gap-2">
          <span class="font-mono text-3xl font-bold text-plum">{{ queueDetail.stats.avgWait }}</span>
    ...
    <div class="space-y-4">
      <div class="flex items-center justify-between">
        ...
        <div class="flex items-center gap-3">
           <div class="relative hidden md:block">
            <SearchIcon class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-plum-muted pointer-events-none" />
            <input
              v-model="searchQuery"
      ...
      <BaseCard class="overflow-hidden border-plum-faint shadow-sm">
          ...
          <table class="w-full text-left">
            <thead>
              <tr class="bg-sand/30 border-b border-plum-faint">
                <th class="px-6 py-4 font-body text-xs font-bold uppercase tracking-wider text-plum-muted">Ticket</th>
                <th class="px-6 py-4 font-body text-xs font-bold uppercase tracking-wider text-plum-muted">Customer Name</th>
                <th class="px-6 py-4 font-body text-xs font-bold uppercase tracking-wider text-plum-muted">Status</th>
                <th class="px-6 py-4 font-body text-xs font-bold uppercase tracking-wider text-plum-muted">Joined At</th>
                <th class="px-6 py-4 font-body text-xs font-bold uppercase tracking-wider text-plum-muted">Wait Time</th>
            ...
            <tbody class="divide-y divide-plum-faint">
              ...
              >
                <td class="px-6 py-5 font-mono text-sm text-plum font-semibold">{{ customer.ticket }}</td>
                <td class="px-6 py-5 font-body text-sm text-plum">{{ customer.name }}</td>
                <td class="px-6 py-5">
                  <BaseBadge :variant="getStatusVariant(customer.status)">{{ customer.status }}</BaseBadge>
                </td>
                ...
                <td class="px-6 py-5 text-right">
                  <button class="p-2 text-plum-muted hover:text-plum transition-colors rounded-lg hover:bg-sand">
        ...
        <div class="px-6 py-4 border-t border-plum-faint bg-sand/5 flex items-center justify-between">
          <p class="font-body text-xs text-plum-muted">
            Showing {{ queueDetail?.entries.length || 0 }} customers from this session
          </p>
          <div class="flex items-center gap-2">
            <button disabled class="w-9 h-9 flex items-center justify-center rounded-xl border border-plum-faint text-plum-muted opacity-30">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
      <BaseCard class="p-6">
        ...
        <p class="font-body text-sm text-plum-muted leading-relaxed">
          {{ queueDetail?.notes || 'No notes were added for this session. You can add notes during the live queue to help with future reporting.' }}
        </p>
      </BaseCard>
      ...
      <BaseCard class="p-6 bg-mint-light/30 border-dashed border-2 border-mint/50">
        ...
        <ul class="space-y-3">
          <li class="flex items-start gap-3">
            <div class="w-5 h-5 rounded-full bg-mint flex items-center justify-center mt-0.5 shrink-0">
              <CheckCircleIcon class="w-3 h-3 text-plum" />
            </div>
            <p class="font-body text-sm text-plum">Your served rate was 15% higher than previous Monday session.</p>
          </li>
          <li class="flex items-start gap-3">
            <div class="w-5 h-5 rounded-full bg-mint flex items-center justify-center mt-0.5 shrink-0">
              <CheckCircleIcon class="w-3 h-3 text-plum" />
            </div>
            <p class="font-body text-sm text-plum">Peak rush occurred between 11:30 AM and 12:15 PM.</p>
          </li>
        </ul>
      </BaseCard>
    </div>
