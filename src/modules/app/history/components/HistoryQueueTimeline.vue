...
<template>
  <div class="rounded-2xl border border-plum/5 bg-white p-6 shadow-[0_1px_2px_rgba(0,0,0,0.05)]">
    <h3 class="font-display text-xl font-bold text-plum">Queue Timeline</h3>

    <div class="relative mt-8 ml-1.5">
      <!-- Vertical line -->
      <div class="absolute left-[5px] top-1 bottom-1 w-0.5 bg-plum/10" />
      ...
      <div class="flex flex-col gap-5">
        <div v-for="(event, idx) in events" :key="idx" class="relative flex gap-6 pl-8">
          <!-- Dot -->
          <div
            :class="['absolute left-0 top-0.5 h-3 w-3 rounded-full border-2', dotColor(event.type)]"
          />
