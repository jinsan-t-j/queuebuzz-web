...
export const API_ROUTES = {
    ...
    CUSTOMER: {
        ...
        JOIN_BY_CODE: (id: string): string => `/queue/${id}/join-by-code`,
        RESOLVE_CODE: (code: string): string => `/queue/resolve-code/${code}`,
        GET_ENTRY: () => `/entry`,
        ENTRY_EVENTS: () => `/entry/events`,
        CONFIRM_STILL_HERE: () => `/entry/confirm`,
        CONFIRM_ARRIVAL: () => `/entry/arrived`,
        LEAVE_GUEST: '/entry/leave',
        RECOVER_SESSION: '/entry/rejoin',
        SUBMIT_RATING: () => `/entry/rating`,
        UPDATE_EMAIL: '/entry/user/email',
        UPDATE_PIN: '/entry/user/pin',
    },
    SHARED: {
